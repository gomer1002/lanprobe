...
| `--packet` | Размер одного пакета в байтах (рекомендуется ≤ MTU сети)                                          | `1400`                |
| `--target` | Целевой IP-адрес клиента (unicast). Если не указан — broadcast (255.255.255.255) (Только для UDP) | `None` (broadcast)    |
...
### 3. UDP unicast (передача на конкретный клиент)
